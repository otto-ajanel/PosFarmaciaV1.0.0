...
class ModeloInventario{
	...
	static public function mdlMostrarProductoInventario($tabla, $item, $valor){
		if($item != null){
			$stmt = Conexion::conectar()->prepare("SELECT t0.CODIGO_PROVEEDOR, t0.CODIGO_PRODUCTO, T0.CODIGO_INVENTARIO, T2.NOMBRE_PROVEEDOR,T1.NOMBRE_GENERICO, T0.STOCK, T0.PRECIO_COMPRA,T0.PRECIO_VENTA, T0.FECHA_INGRESO, T0.CADUCIDAD
                                              FROM INVENTARIO T0
                                              JOIN PRODUCTO T1 ON T1.CODIGO_PRODUCTO = T0.CODIGO_PRODUCTO
                                              JOIN PROVEEDOR T2 ON T2.CODIGO_PROVEEDOR = T0.CODIGO_PROVEEDOR
                                              WHERE T1.VISIBLE = 1 AND T2.VISIBLE = 1 AND t0.CODIGO_INVENTARIO= :item");
			$stmt -> bindParam(":item", $valor, PDO::PARAM_INT);
			$stmt -> execute();
			return $stmt -> fetch();
		}else{
			$stmt = Conexion::conectar()->prepare("SELECT T0.CODIGO_INVENTARIO, t0.CODIGO_PROVEEDOR, t0.CODIGO_PRODUCTO, T2.NOMBRE_PROVEEDOR, T1.NOMBRE_GENERICO,  T0.CODIGO_BARRA, T0.STOCK, T0.PRECIO_COMPRA, T0.PRECIO_VENTA, T0.FECHA_INGRESO, T0.CADUCIDAD
                                              FROM INVENTARIO T0
                                              JOIN PRODUCTO T1 ON T1.CODIGO_PRODUCTO = T0.CODIGO_PRODUCTO
                                              JOIN PROVEEDOR T2 ON T2.CODIGO_PROVEEDOR = T0.CODIGO_PROVEEDOR

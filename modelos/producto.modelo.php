...
class ModeloProducto{
	...
	static public function mdlIngresarProducto($tabla, $datos){
		...
		$stmt = $db->prepare("INSERT INTO $tabla (/*CODIGO_BARRAS,*/ NOMBRE_GENERICO, NOMBRE_COMERCIAL, CODIGO_PRESENTACION, CODIGO_TIPO, STOCK_MIN, STOCK_MAX,URL,UBICACION_PRODUCTO) VALUES (/*:nuevoCodigoBarras,*/ :nuevoNombreGenerico, :nuevoNombreComercial, :presentacionProducto , :tipoproductoProducto , :nuevoStockMinimo, :nuevoStockMaximo, :url, :ubicacion)");
	/*	$stmt->bindParam(":nuevoCodigoBarras", $datos["CODIGO_BARRAS"], PDO::PARAM_INT);*/
		...
		$stmt->bindParam(":presentacionProducto", $datos["CODIGO_PRESENTACION"], PDO::PARAM_INT);
		$stmt->bindParam(":tipoproductoProducto", $datos["CODIGO_TIPO"], PDO::PARAM_INT);
		$stmt->bindParam(":nuevoStockMinimo", $datos["STOCK_MIN"], PDO::PARAM_INT);
		$stmt->bindParam(":nuevoStockMaximo", $datos["STOCK_MAX"], PDO::PARAM_INT);
		$stmt->bindParam(":ubicacion", $datos["UBICACION_PRODUCTO"], PDO::PARAM_STR);
		$stmt->bindParam(":url", $datos["URL"], PDO::PARAM_STR);

		if($stmt->execute()){
		
			return $db->lastInsertId("CODIGO_PRODUCTO");
		}else{
			return "error";
		}
		$stmt->close();
	...
	static public function mdlMostrarProducto($tabla, $item, $valor){
		if($item != null){
			$stmt = Conexion::conectar()->prepare("SELECT t0.CODIGO_PRODUCTO, t0.CODIGO_PRESENTACION, t0.CODIGO_TIPO, t0.NOMBRE_GENERICO, t0.NOMBRE_COMERCIAL,  t3.TIPO_PRODUCTO, t0.STOCK_MIN, t0.STOCK_MAX /*t0.CODIGO_BARRAS,*/
																								FROM producto t0
			...
			$stmt = Conexion::conectar()->prepare("SELECT t0.CODIGO_PRODUCTO, t0.CODIGO_PRESENTACION, t0.CODIGO_TIPO, /*t0.CODIGO_BARRAS,*/ t0.NOMBRE_GENERICO, t0.NOMBRE_COMERCIAL, t1.PRESENTACION, t3.TIPO_PRODUCTO, t0.STOCK_MIN, t0.STOCK_MAX
																								FROM producto t0
																								JOIN presentacion t1 ON t1.CODIGO_PRESENTACION = t0.CODIGO_PRESENTACION
	...
	static public function mdlMostrarInventario($tabla){
		$stmt=Conexion::conectar()->prepare("SELECT CODIGO_INVENTARIO,PRECIO_VENTA, CODIGO_BARRA, STOCK,NOMBRE_GENERICO,URL,TIPO_PRODUCTO,PRESENTACION FROM $tabla I
